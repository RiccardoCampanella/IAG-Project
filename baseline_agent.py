...
class FakeNewsAgent:
    ...
    def deactivate_goals(self):
        logging.debug(f"Deactivating prior goals")
        for goal in self.subgoals:
            ...
            if goal.plan.steps[-1] == self.state: # assume the plan list is ordered in sequential logical order
                 ...
                 # if a goal is not reached it can be resumed later (unless unachievable)
    ...
    def adopt_active_goals(self) -> None:
        ...
        logging.debug(f"Adopt_active_goals: {active_goals}")
